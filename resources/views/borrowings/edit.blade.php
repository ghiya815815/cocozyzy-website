...
<main>
    <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-bold text-black dark:text-white">Edit Peminjaman #{{ $borrowing->id }}</h2>
        ...
        <div class="rounded-sm border border-stroke bg-white p-6 shadow-default dark:border-strokedark dark:bg-boxdark">
            <form action="{{ route('borrowings.update', $borrowing->id) }}" method="POST">
                    ...
                    <div class="relative z-20 bg-white dark:bg-form-input">
                        <select id="book_id" name="book_id"
                            ...
                            @foreach ($books as $book)
                                <option value="{{ $book->id }}" {{ old('book_id', $borrowing->book_id) == $book->id ? 'selected' : '' }}>{{ $book->title }} (Stok: {{ $book->quantity }})</option>
                            @endforeach
                        </select>
                        <span class="absolute top-1/2 right-4 -translate-y-1/2">
                            <svg class="fill-current" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <div class="mb-4">
                    ...
                    <input type="date" id="borrow_date" name="borrow_date" value="{{ old('borrow_date', \Carbon\Carbon::parse($borrowing->borrow_date)->format('Y-m-d')) }}"
                ...
                <div class="mb-4">
                    ...
                    <input type="date" id="due_date" name="due_date" value="{{ old('due_date', \Carbon\Carbon::parse($borrowing->due_date)->format('Y-m-d')) }}"
                        class="w-full rounded border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary" required>
                </div>
                <div class="mb-6">
                    <label for="returned_at" class="mb-3 block text-black dark:text-white">Tanggal Pengembalian</label>
                    <input type="date" id="returned_at" name="returned_at" value="{{ old('returned_at', $borrowing->returned_at ? \Carbon\Carbon::parse($borrowing->returned_at)->format('Y-m-d') : '') }}"
                        class="w-full rounded border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary">
                    <small class="text-gray-500 dark:text-gray-400">Biarkan kosong jika belum dikembalikan.</small>
                </div>
